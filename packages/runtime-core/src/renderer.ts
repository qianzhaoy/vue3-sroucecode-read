...
function baseCreateRenderer(
  ...
  // tip: 下面是一些基于 renderOptions 的一些方法抽象
  ...
  const mountComponent: MountComponentFn = (
    ...
    // tip: 处理 setup 函数, 并将结果赋值 instance.setupState
    ...
    // tip: 给组件赋值 update 生命周期, 并调用 update 渲染到 UI
    setupRenderEffect(
      instance,
      initialVNode,
      container,
  ...
  const setupRenderEffect: SetupRenderEffectFn = (
    ...
    // 因为 effect 没有 lazy 所以会直接调用一次 update 相当于 mount
  ...
  const unmountComponent = (
    ...
    // tip: 卸载依赖的 computed 和 watch effect
    if (effects) {
    ...
    // setup has resolved.
    if (update) {
      // tip: 卸载 reactive 依赖的 componentUpdate effect 
      stop(update)
  ...
  // 划重点: render函数第一个参数 vnode, 第二个为 container
